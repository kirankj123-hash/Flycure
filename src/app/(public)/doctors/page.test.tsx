...
describe('DoctorsPage', () => {
  ...
  test('renders page title with correct heading', () => {
    ...
    expect(heading).toHaveTextContent('Expert Doctors');
  });
  ...
  test('renders all doctor feature cards', () => {
    render(<DoctorsPage />);
    
    // Test for key doctors using heading role
    expect(screen.getByRole('heading', { level: 3, name: 'Dr. Sarah Johnson' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: 'Dr. Michael Chen' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: 'Dr. Emily Rodriguez' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: 'Dr. Ahmed Hassan' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: 'Dr. Lisa Thompson' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: 'Dr. Robert Kim' })).toBeInTheDocument();
  });

  test('renders doctor specializations and descriptions', () => {
    render(<DoctorsPage />);
    
    expect(screen.getByText(/Leading Cardiac Surgeon with 15\+ years experience/)).toBeInTheDocument();
    expect(screen.getByText(/Renowned Orthopedic Specialist focusing on joint replacement/)).toBeInTheDocument();
    expect(screen.getByText(/Expert Oncologist specializing in personalized cancer treatment/)).toBeInTheDocument();
    expect(screen.getByText(/Leading Neurologist with expertise in brain surgery/)).toBeInTheDocument();
  });
  ...
  test('renders all 6 doctors with correct data structure', () => {
    render(<DoctorsPage />);
    ...
    doctors.forEach(doctor => {
      expect(screen.getByRole('heading', { level: 3, name: doctor })).toBeInTheDocument();
    });
  });

  test('displays diverse medical specialties', () => {
    render(<DoctorsPage />);
    
    // Check that different medical specialties are represented
    expect(screen.getByText(/Cardiac Surgeon/)).toBeInTheDocument();
    expect(screen.getByText(/Orthopedic Specialist/)).toBeInTheDocument();
    expect(screen.getByText(/Oncologist/)).toBeInTheDocument();
    expect(screen.getByText(/Neurologist/)).toBeInTheDocument();
    expect(screen.getByText(/Gastroenterology expert/)).toBeInTheDocument();
  ...
  test('hero section highlights key value proposition', () => {
    render(<DoctorsPage />);
    
    // Should emphasize the quality and expertise of doctors
    expect(screen.getByText(/world-class medical professionals/)).toBeInTheDocument();
    expect(screen.getByText(/specialists/)).toBeInTheDocument();
  });

  test('contains relevant medical terminology', () => {
    render(<DoctorsPage />);
    
    // Should contain relevant medical terms and procedures
    expect(screen.getByText(/minimally invasive/)).toBeInTheDocument();
    expect(screen.getByText(/immunotherapy/)).toBeInTheDocument();
    expect(screen.getByText(/endoscopic procedures/)).toBeInTheDocument();
    expect(screen.getByText(/brain surgery/)).toBeInTheDocument();
  });
});